...
describe("flow-add-step", () => {
  ...
  it("runs a leading restart-app live but does not record it on an e2e flow", async () => {
    const registry = createMockRegistry({
    ...
    await flowStartRecordingTool.execute(
      {},
      { name: "e2e-launch", project_root: tmpDir, launch: "com.acme.app" }
    );
    ...
    // Ran live (so the device is set up for the rest of the recording)…
    expect(result.toolResult).toEqual({ restarted: true });
    expect(registry.invokeTool).toHaveBeenCalledWith("restart-app", {
      udid: "ABC",
      bundleId: "com.acme.app",
    });
    expect(result.message).toMatch(/did not record/i);
    // …but it is not in the recorded steps (the runner relaunches at replay).
    expect(parseFlow(result.flowFile).steps).toEqual([]);
  });

  it("records a restart-app that is not the leading step", async () => {
    const registry = createMockRegistry({
      tap: { result: { tapped: true } },
      "restart-app": { result: { restarted: true } },
    });
    ...
    await flowStartRecordingTool.execute(
      {},
      { name: "mid-restart", project_root: tmpDir, launch: "com.acme.app" }
    );
    await tool.execute({}, { command: "tap", args: '{"x":0.5,"y":0.3}' });
    const result = await tool.execute(
      {},
      { command: "restart-app", args: '{"udid":"ABC","bundleId":"com.acme.app"}' }
    );
    ...
    expect(parseFlow(result.flowFile).steps).toEqual([
      ...
      { kind: "tool", name: "restart-app", args: { bundleId: "com.acme.app" } },
    ]);
  });

  it("records a leading restart-app on a fragment (no launch block to relaunch)", async () => {
    const registry = createMockRegistry({
      "restart-app": { result: { restarted: true } },
    });
    const tool = createFlowAddStepTool(registry);
    ...
    await flowStartRecordingTool.execute(
      {},
      { name: "frag-restart", project_root: tmpDir, executionPrerequisite: PREREQ }
    );
    const result = await tool.execute(
      {},
      { command: "restart-app", args: '{"udid":"ABC","bundleId":"com.acme.app"}' }
