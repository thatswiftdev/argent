...
Every other recorded tool (`gesture-swipe`, `gesture-scroll`, `button`, `screenshot`, …) has no directive form — leave it as a `tool:` step. The recorder already handles the rest: coordinate `gesture-tap`s are captured as portable `tap:` selector steps, a `flow-execute` of a sibling fragment is captured as a `run: <name>` composition directive, device ids are stripped, text-only selectors are emitted as bare strings, and a leading `restart-app`/`launch-app` is dropped. After editing, re-run with `flow-execute` to confirm the cleaned flow still passes.
